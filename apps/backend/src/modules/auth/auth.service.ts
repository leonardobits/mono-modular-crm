...
export class AuthService {
  ...
  async registerInitialAdmin(registerDto: InitialAdminRegisterDto) {
    ...
    try {
      ...
      if (countError) {
        throw new BadRequestException(`Error checking existing users: ${countError.message}`);
      }

      const isFirstUser = userCount === 0;

      if (!isFirstUser) {
        throw new BadRequestException('Initial admin registration is only allowed for the first user. Admin already exists.');
      }
      ...
      return {
        success: true,
        message: 'Initial admin registration successful. Please check your email for verification.',
        user: {
          id: authData.user.id,
      ...
      throw new BadRequestException('Registration failed due to unexpected error');
    }
  }
  ...
  async resetPassword(resetPasswordDto: ResetPasswordDto) {
    ...
    try {
      ...
      if (profile.status !== 'ACTIVE') {
        console.log(`Password reset requested for inactive user: ${resetPasswordDto.email}`);
        return {
          success: true,
          message: 'If this email exists in our system, a password reset email has been sent.'
        };
      ...
      throw new BadRequestException('Password reset failed due to unexpected error');
    }
  }
  ...
  async login(loginDto: LoginDto) {
    ...
    try {
      ...
      if (authError) {
        throw new UnauthorizedException(`Authentication failed: ${authError.message}`);
      }
