...
export class UsersService {
    ...
    try {
      ...
      if (usersError) {
        ...
        throw new InternalServerErrorException(
          'Could not fetch authentication users.',
        );
      }
    ...
    try {
      ...
      if (authError || !authUser) {
        throw new NotFoundException(
          `Authentication user with ID ${id} not found.`,
        );
      }
  ...
  async create(createUserDto: CreateUserSchema) {
    ...
    try {
      ...
      if (authError) {
        ...
        throw new InternalServerErrorException(
          'Could not create user in authentication system.',
        );
      }

      if (!authData?.user) {
        throw new InternalServerErrorException(
          'User was not created in authentication system, but no error was reported.',
        );
      }
      ...
      throw new InternalServerErrorException(
        'User creation failed due to unexpected error.',
      );
    }
  }
  ...
  async update(id: string, updateUserDto: UpdateUserSchema) {
    ...
    try {
      const { data: authUser, error: findError } =
  ...
  async updatePassword(id: string, updatePasswordDto: { password?: string }) {
    ...
    try {
      ...
      console.error(
        `Unexpected error in updatePassword for user ID ${id}:`,
        error,
      );
      throw new InternalServerErrorException(
        'Password update failed due to an unexpected error.',
      );
    }
  }
  ...
  private async updateUserStatus(id: string, status: 'ACTIVE' | 'INACTIVE') {
    ...
    try {
      ...
      if (error) {
        // If the profile is not found, we'll create it.
        if (error.code === 'PGRST116') {
          console.warn(`Profile for user ${id} not found. Creating a new one.`);

          // Fetch auth user to get essential details like email.
          const { data: authUser, error: authError } =
            await adminSupabase.auth.admin.getUserById(id);
          if (authError || !authUser) {
            throw new NotFoundException(
              `Authentication user with ID ${id} not found, cannot create profile.`,
            );
          }
          ...
          const { error: insertError } = await adminSupabase
            ...
            .insert({
              ...
              status,
              email: authUser.user.email,
              full_name: authUser.user.email, // Use email as a placeholder for full_name
              role: 'USER', // Assign a default role
            });

          if (insertError) {
            console.error(
              `Failed to create missing profile for user ${id}:`,
              insertError,
            );
            throw new InternalServerErrorException(
              'Could not create missing user profile.',
            );
          }
        } else {
          // For other errors, re-throw.
          console.error(
            `Error updating status for user ${id} to ${status}:`,
            error,
          );
          throw new InternalServerErrorException(
      ...
      console.error(
        `Unexpected error in updateUserStatus for user ID ${id}:`,
        error,
      );
      throw new InternalServerErrorException(
        'User status update failed unexpectedly.',
      );
    }
  }
